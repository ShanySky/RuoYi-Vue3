...
async function selectPersonalDefaults(settings, targetPage = page, modelName = 'mock-agent-model', effortName = '高') {
  const modelRow = settings.locator('.preference-row').filter({ hasText: '我的默认模型' })
  await modelRow.locator('.el-select').click()
  await targetPage.locator('.el-select-dropdown:visible').getByText(modelName, { exact: true }).click()
...
try {
  ...
  console.log('5a. Model advanced settings prefer recommended mode, support custom values and restore recommendation')
  ...
  await runtimeDialog.getByText('Agent 工作上下文预算', { exact: true }).first().waitFor()
  const recommendedSummary = runtimeDialog.getByTestId('ai-runtime-recommended-summary')
  await recommendedSummary.waitFor()
  assert.match(await recommendedSummary.innerText(), /64K tokens/)
  assert.match(await recommendedSummary.innerText(), /75%/)
  assert.equal(await runtimeDialog.getByTestId('ai-runtime-custom-settings').count(), 0)

  await runtimeDialog.getByText('自定义', { exact: true }).click()
  await runtimeDialog.getByTestId('ai-runtime-custom-settings').waitFor()
  ...
  let autoCompactionSwitch = runtimeDialog.locator('.el-switch').first()
  ...
  assert.equal(await runtimeInputs.nth(0).inputValue(), '72')
  ...
  assert.equal(persistedPrimary.compactionThresholdPercent, 75)
  assert.equal(persistedPrimary.autoCompaction, '0')
  ...
  await page.getByTestId('ai-assistant-new-conversation').click()
  await page.getByRole('button', { name: '继续上次会话', exact: true }).waitFor({ timeout: 10000 })
  ...
  await settings.getByText('登录后自动继续上次会话', { exact: true }).waitFor()
  ...
  let defaultThresholdConversationId = null
  let defaultThresholdLastTurn = null
  const thresholdPayload = 'x'.repeat(11900)
  for (let i = 1; i <= 14; i++) {
    defaultThresholdLastTurn = await apiJson(token, '/ai/chat/turn', 'POST', {
      ...(defaultThresholdConversationId ? { conversationId: defaultThresholdConversationId } : {}),
      modelId: primaryModel.modelId,
      reasoningEffort: 'high',
      userMessage: `I_DEFAULT_64K_75_BELOW_${i} ${thresholdPayload}`,
      route: '/index',
      pageContext: {},
      frontendTools: []
    })
    assert.equal(defaultThresholdLastTurn.type, 'MESSAGE')
    defaultThresholdConversationId ||= defaultThresholdLastTurn.conversationId
  }

  const belowDefaultThreshold = await apiJson(token, `/ai/admin/audit/${defaultThresholdConversationId}`)
  assert.equal((belowDefaultThreshold.checkpoints || []).length, 0,
    'Fourteen max-sized persisted turns must remain below the configured 64K/75% threshold')

  let afterDefaultThreshold = belowDefaultThreshold
  for (let i = 15; i <= 18 && (afterDefaultThreshold.checkpoints || []).length === 0; i++) {
    defaultThresholdLastTurn = await apiJson(token, '/ai/chat/turn', 'POST', {
      conversationId: defaultThresholdConversationId,
      modelId: primaryModel.modelId,
      reasoningEffort: 'high',
      userMessage: `I_DEFAULT_64K_75_CROSS_${i} ${thresholdPayload}`,
      route: '/index',
      pageContext: {},
      frontendTools: []
    })
    assert.equal(defaultThresholdLastTurn.type, 'MESSAGE')
    afterDefaultThreshold = await apiJson(token, `/ai/admin/audit/${defaultThresholdConversationId}`)
  }

  assert.ok((afterDefaultThreshold.checkpoints || []).length >= 1,
    'Crossing the default 64K/75% threshold with persisted 12k-char turns must create an automatic checkpoint')
  ...
  assert.ok((afterDefaultThreshold.runs || []).some(run =>
    run.status === 'COMPLETED' && Number(run.runId) === Number(defaultThresholdLastTurn.runId)),
