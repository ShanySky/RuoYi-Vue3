...
function requireText(value, name, maxLength) {
  ...
  if (maxLength && normalized.length > maxLength) {
    throw new Error(`Page Capability Contract ${name} 长度超过 ${maxLength}`)
  }
  return normalized
}
...
export function definePageCapabilityContract(definition) {
  ...
  const pageId = requireText(definition.pageId, 'pageId', 128)
  const pageName = requireText(definition.pageName, 'pageName')
  const route = requireText(definition.route, 'route', 255)
