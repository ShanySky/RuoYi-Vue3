...
- 前后端联动改动在当前能力允许时必须检查 `ShanySky/RuoYi-Vue` 的协议、权限和当前实现；当前上下文无法访问后端时，不编造检查结果，也不因此擅自切换执行环境。
- UI / Agent 交互不能只以 `npm build` 成功作为完成依据；能自动验证的尽量在当前执行上下文内实际运行。
...
1. **当前事实**：当前执行上下文、目标分支 / Working Tree、相关前端代码、配置、测试。
2. **本文件**：确认前端边界和任务路由。
...
该文件是项目级规则的本地 mirror；canonical source 位于后端同路径。先判断当前任务实际建立在哪个执行上下文，再依据真实能力选择本地命令、GitHub API、Actions、远程环境或其他执行方式。
...
跨仓库验收必须记录实际使用的前后端 branch / SHA；如果验证的是未提交本地修改，则明确记录该事实而不是虚构 SHA。不要因为后端 workflow 变绿就默认当前前端工作分支也被测试；先检查 workflow 实际 checkout 的 frontend ref。验证深度不构成切换执行环境的授权。
